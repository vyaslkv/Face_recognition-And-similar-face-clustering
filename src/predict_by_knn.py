...
def predict(X_img_path, knn_clf=None, model_path=None, distance_threshold=0.4):
    ...
    X_img=X_img[:,600:1500]
    import cv2
    cv2.imshow("dfdf",X_img)
    cv2.waitKey(0)
...
imagePaths = list(paths.list_images('dataset_alpr'))
count=0
for image in imagePaths:
    print(image)
    name_id= image.split('/')[1]
    predictions = predict(image,model_path='knn_trained_hog_4neigh_1test_train-80')
